...
const Dashboard = ({ params }) => {
  ...
  const fetchOppData = async (userID) => {
    try {
      // Fetch opponent data
      const { data: userData, error: userError } = await supabase
        .from("users")
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        await fetchUserData(params.user_ID);
        await fetchOppData(params.sUserID);
      } catch (error) {
  ...
  const handleBack = () => {
    router.push(`/landing/${params.user_ID}/kickoff`);
  };
  ...
  useEffect(() => {
    ...
    const updateScore = async (userID: any) => {
      try {
        ...
        // Calculate the new score based on the result
        let newScore = userData.score + 5;
    ...
    // Call the updateScore function based on the result
    if (result === "Win") {
      updateScore(params.user_ID);
    } else if (result === "Lose") {
      updateScore(params.sUserID);
    }
